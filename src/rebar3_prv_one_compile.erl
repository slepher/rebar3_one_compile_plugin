...
init(State) ->
    ...
    State1 = rebar_state:add_provider(
               State, providers:create([{name, ?PROVIDER},
                                        ...
                                        {bare, true},
                                        {deps, ?DEPS},
                                        {example, "rebar3 one compile -a $APP -m $MODULE"},
                                        {short_desc, "Compile .erl files."},
                                        {desc, "Compile .erl files."},
                                        {opts, Opts}])),
    {ok, State1}.
...
run(CompilerMod, AppInfo, Module, Label) ->
    ...
    FoundFiles = rebar3_one_compile_dag_lib:find_source_files(BaseDir, SrcExt, SrcDirs, BaseOpts),
